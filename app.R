...
accepted_color_scales <- tribble(~shortcode, ~ggplot,
                                 'viridis' , viridis::scale_fill_viridis(discrete = TRUE),
                                 'ggsci_d3_cat20', ggsci::scale_color_d3("category20"),
                                 'ggsci_npg', ggsci::scale_fill_npg(),
                                 'iwanthue', hues::scale_fill_iwanthue())
        ...
        mainPanel(
            tabsetPanel(
                ...
                tabPanel("Heatmap of transcript expression",
                         ...
                         plotOutput("heatmapPhmp", width = "100%")
...
server <- function(input, output, session) {
    ...
    output$tx_neuron_proportions <- renderPlotly({
        ...
        ind_graph <- tx_long %>%
            ...
            # guides(fill=guide_legend(title=my_gene_name_1())) +
